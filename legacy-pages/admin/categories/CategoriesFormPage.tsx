...
interface CategoriesFormPageProps {
  FormComponent: ComponentType<CategoryFormProps>;
  title: string;
}
...
export default function CategoriesFormPage({
  FormComponent,
  title,
}: CategoriesFormPageProps) {
  const dispatch = useAppDispatch() as unknown as LegacyDispatch;
  const router = useRouter();
  const form = useAppSelector((state) => state.categories.form);
